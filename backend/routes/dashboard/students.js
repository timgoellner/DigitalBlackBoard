...
router.post("/", async (request, response) => {
  ...
  const grade = await mysql.sendQuery(`SELECT grades.id FROM grades WHERE organization = ${escape(user.organization)} AND grade = ${escape(studentGrade)} AND subgrade ${studentSubgrade}`)
  if (grade.length === 0) return response.status(401).json({ message: 'grade does not exist' })

  var identifier = await mysql.sendQuery(`SELECT identifier FROM accounts WHERE organization = ${escape(user.organization)} AND isStaff = FALSE ORDER BY identifier DESC LIMIT 1`)
  identifier = String(parseInt((!identifier) ? '0' : identifier[0].identifier) + 1).padStart(5, '0')

  const accountId = JSON.parse(JSON.stringify(await mysql.sendQuery(`INSERT INTO accounts (organization, identifier, isStaff) VALUES (${escape(user.organization)}, ${escape(identifier)}, 0)`))).insertId

  await mysql.sendQuery(`INSERT INTO students (organization, account, forename, lastname, grade) VALUES (${escape(user.organization)}, ${escape(accountId)}, ${escape(studentForename)}, ${escape(studentLastname)}, ${escape(grade[0].id)})`)
...
router.put("/", async (request, response) => {
  ...
  const existing = await mysql.sendQuery(`SELECT * FROM students WHERE organization = ${escape(user.organization)} AND id = ${escape(id)}`)
  ...
  await mysql.sendQuery(`UPDATE students SET grade = ${escape(grade[0].id)} WHERE id = ${escape(id)}`)
...
router.delete("/", async (request, response) => {
  ...
  await mysql.sendQuery(`DELETE FROM students WHERE organization = ${escape(user.organization)} AND id = ${escape(id)}`)
  await mysql.sendQuery(`DELETE FROM accounts WHERE organization = ${escape(user.organization)} AND id = ${escape(existing[0].account)}`)
