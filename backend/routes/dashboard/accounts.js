...
router.get("/", async (request, response) => {
  ...
  const accounts = await mysql.sendQuery(`SELECT * FROM accounts WHERE organization = '${user.organization}' AND isStaff = 1`)
...
router.post("/", async (request, response) => {
  ...
  const account = await mysql.sendQuery(`SELECT * FROM accounts WHERE organization = '${user.organization}' AND identifier = '${identifier}'`)
  if (account.length > 0) return response.status(401).json({ message: 'account already exists' })

  password = await bcypt.hash(password, 10)

  await mysql.sendQuery(`INSERT INTO accounts (organization, identifier, password, isStaff) VALUES ('${user.organization}', '${identifier}', '${password}', 1)`)
...
router.put("/", async (request, response) => {
  ...
  const account = await mysql.sendQuery(`SELECT * FROM accounts WHERE organization = '${user.organization}' AND id = ${id}`)
  if (account.length === 0) return response.status(401).json({ message: 'account does not exist' })

  password = await bcypt.hash(password, 10)

  await mysql.sendQuery(`UPDATE accounts SET password = '${password}' WHERE organization = '${user.organization}' AND id = ${id}`)
...
router.delete("/", async (request, response) => {
  ...
  const existing = await mysql.sendQuery(`SELECT * FROM accounts WHERE organization = '${user.organization}' AND id = ${id}`)
  if (existing.length === 0) return response.status(401).json({ message: 'account does not exist' })

  await mysql.sendQuery(`DELETE FROM accounts WHERE organization = '${user.organization}' AND id = ${id}`)
